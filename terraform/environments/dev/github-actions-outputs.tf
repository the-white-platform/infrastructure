# Outputs for GitHub Actions WIF configuration
...
output "github_actions_wif_setup_instructions" {
  ...
  value = <<-EOT
    ...
    WIF_PROVIDER_DEV = ${local.wif_provider_name}
    WIF_SERVICE_ACCOUNT_DEV = ${google_service_account.github_actions_deployer.email}
